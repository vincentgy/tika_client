...
const UpdateModuleTemp = UpdateModule
  ? UpdateModule
  : {
      documentPath: () => void 666,
      jsBundleFile: '',
    };
const {documentPath, jsBundleFile} = UpdateModuleTemp;
...
function getUpdateConfigFromServer() {
  return new Promise(resolve => {
    net.get(
      `${UPDATE_HOST}/update.${Platform.OS}.json`,
      result => {
        resolve(result);
      },
      () => {
...
function getUpdateConfigFromLocal(updateConfigFromServer) {
  ...
  return new Promise(resolve => {
    fs.readFile(localVersionPath, 'utf8')
      .then(text => {
      ...
      if (localVersion.appV < serverVersion.appV) {
        ...
        if (localJSVersion < serverJSVersion) {
          ...
          if (localJSVersion === 0) {
            ...
            resolve(
              serverVersion.versionList[`${localJSVersion}-${serverJSVersion}`]
            );
          }
        } else {
...
function patchApply() {
  return new Promise((resolve, reject) => {
    if (__DEV__) {
      ...
      UpdateModule.patchApply(
        Math.max(1, localVersion.jsV),
        serverVersion.jsV,
        () => {
          resolve(JSON.stringify(serverVersion));
        },
...
function initPatchList() {
  ...
  for (k in localVersion.versionList) {
    global.patchList = global.patchList.concat(
      localVersion.versionList[k].patchList
    );
  }
  // 去重
...
function saveVersion(content) {
  return new Promise((resolve, reject) => {
    fs.writeFile(localVersionPath, content, 'utf8')
      .then(() => {
        resolve();
      })
      .catch(() => {
...
function removePatch() {
  return new Promise(resolve => {
    const patchPath = `${documentPath}/${Math.max(1, localVersion.jsV)}-${
      serverVersion.jsV
    }.jsPatches`;
    fs.unlink(patchPath)
      .then(() => {
